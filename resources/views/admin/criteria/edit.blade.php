<x-layouts::app :title="__('Criterium bewerken')">
            ...
            <form action="{{ route('admin.criteria.update', $criterion) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    >
                        ...
                        @foreach($standards as $standard)
                            <option value="{{ $standard->id }}" {{ old('standard_id', $criterion->standard_id) == $standard->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    <input
                        ...
                        name="number"
                        id="number"
                        value="{{ old('number', $criterion->number) }}"
                        min="1"
                        class="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2 text-sm text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-100"
                        required
                    />
                ...
                <div class="mb-4">
                    ...
                    <textarea
                        ...
                        class="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2 text-sm text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-100"
                        required
                    >{{ old('text', $criterion->text) }}</textarea>
                    @error('text')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea
                        ...
                        rows="4"
                        class="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2 text-sm text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-100"
                    >{{ old('explanation', $criterion->explanation) }}</textarea>
                    @error('explanation')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
