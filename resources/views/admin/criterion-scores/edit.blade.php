<x-layouts::app :title="__('Criteriumscore bewerken')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Criteriumscore bewerken</h1>
        ...
        <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            <form action="{{ route('admin.criterion-scores.update', $criterionScore) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    >
                        ...
                        @foreach($criteria as $criterion)
                            <option value="{{ $criterion->id }}" {{ old('criterion_id', $criterionScore->criterion_id) == $criterion->id ? 'selected' : '' }}>
                ...
                <div class="mb-4">
                    ...
                    >
                        ...
                        @foreach($reportingPeriods as $period)
                            <option value="{{ $period->id }}" {{ old('reporting_period_id', $criterionScore->reporting_period_id) == $period->id ? 'selected' : '' }}>
                                {{ $period->label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    >
                        ...
                        <option value="sufficient"   {{ old('status', $criterionScore->status) === 'sufficient'   ? 'selected' : '' }}>Voldoende</option>
                        <option value="attention"    {{ old('status', $criterionScore->status) === 'attention'    ? 'selected' : '' }}>Aandacht</option>
                        <option value="insufficient" {{ old('status', $criterionScore->status) === 'insufficient' ? 'selected' : '' }}>Onvoldoende</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
