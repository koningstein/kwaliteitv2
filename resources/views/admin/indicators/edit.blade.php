<x-layouts::app :title="__('Indicator bewerken')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Indicator bewerken</h1>
            ...
            <form action="{{ route('admin.indicators.update', $indicator) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    >
                        ...
                        @foreach($criteria as $criterion)
                            <option value="{{ $criterion->id }}" {{ old('criterion_id', $indicator->criterion_id) == $criterion->id ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <textarea
                        ...
                        class="w-full rounded-lg border border-zinc-300 bg-white px-4 py-2 text-sm text-zinc-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-zinc-600 dark:bg-zinc-700 dark:text-zinc-100"
                        required
                    >{{ old('text', $indicator->text) }}</textarea>
                    @error('text')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
