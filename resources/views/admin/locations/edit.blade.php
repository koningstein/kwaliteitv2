<x-layouts::app :title="__('Locatie bewerken')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Locatie bewerken: {{ $location->name }}</h1>
            <a href="{{ route('admin.locations.index') }}" class="text-sm text-zinc-600 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100" wire:navigate>
        ...
        <div class="rounded-lg border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            <form action="{{ route('admin.locations.update', $location) }}" method="POST">
                ...
                <div class="mb-6">
                    <label for="abbreviation" class="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Afkorting</label>
                    <input
                        type="text"
                        name="abbreviation"
                        id="abbreviation"
                        value="{{ old('abbreviation', $location->abbreviation) }}"
                        maxlength="10"
