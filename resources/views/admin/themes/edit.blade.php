<x-layouts::app :title="__('Thema bewerken')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Thema bewerken: {{ $theme->name }}</h1>
                ...
                <div class="mb-4">
                    ...
                    <div class="flex items-center gap-3">
                        <input
                            ...
                            name="color"
                            id="color"
                            value="{{ old('color', $theme->color) }}"
                            class="h-10 w-14 cursor-pointer rounded border border-zinc-300 bg-white p-1 dark:border-zinc-600 dark:bg-zinc-700"
                        />
                        <span id="color-value" class="text-sm text-zinc-500 dark:text-zinc-400">{{ old('color', $theme->color) }}</span>
                    </div>
                    @error('color')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    <label class="flex items-center gap-2">
                        ...
                        <input
                            ...
                            name="is_deletable"
                            value="1"
                            {{ old('is_deletable', $theme->is_deletable) ? 'checked' : '' }}
