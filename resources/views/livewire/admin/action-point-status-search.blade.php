                    ...
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800">
                        ...
                        <td class="whitespace-nowrap px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.action-point-statuses.edit', $status) }}" class="inline-flex items-center rounded-md bg-yellow-50 px-3 py-1.5 text-xs font-medium text-yellow-700 ring-1 ring-inset ring-yellow-600/20 hover:bg-yellow-100 dark:bg-yellow-900/30 dark:text-yellow-300 dark:ring-yellow-500/30 dark:hover:bg-yellow-900/50" wire:navigate>
                                    Bewerken
                                </a>
                                <form action="{{ route('admin.action-point-statuses.destroy', $status) }}" method="POST" onsubmit="return confirm('Weet je zeker dat je deze status wilt verwijderen?')">
                                    @csrf
                                    @method('DELETE')
