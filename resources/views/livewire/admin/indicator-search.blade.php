<div>
    <div class="mb-4">
        <input
            ...
            placeholder="Zoek op tekst, criterium of standaard..."
        ...
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
            <thead class="bg-zinc-50 dark:bg-zinc-800">
                <tr>
                    ...
                    <th class="w-24 px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Standaard</th>
                ...
                @forelse($indicators as $indicator)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800">
                        <td class="px-6 py-4 text-sm font-medium text-zinc-900 dark:text-zinc-100">
                            <div class="max-w-xs truncate" title="{{ $indicator->text }}">
                                {{ Str::limit($indicator->text, 50) }}
                            </div>
                        </td>
                        <td class="whitespace-nowrap px-6 py-4">
                            ...
                            <span class="ml-1 text-sm text-zinc-500 dark:text-zinc-400">{{ Str::limit($indicator->criterion->text, 30) }}</span>
                        </td>
                        <td class="whitespace-nowrap px-6 py-4">
                            <span class="inline-flex items-center rounded-md bg-blue-50 px-2 py-1 text-xs font-medium text-blue-700 ring-1 ring-inset ring-blue-600/20 dark:bg-blue-900/30 dark:text-blue-300 dark:ring-blue-500/30">
                                {{ $indicator->criterion->standard->code }}
                            </span>
                        </td>
                        <td class="whitespace-nowrap px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.indicators.edit', $indicator) }}" class="inline-flex items-center rounded-md bg-yellow-50 px-3 py-1.5 text-xs font-medium text-yellow-700 ring-1 ring-inset ring-yellow-600/20 hover:bg-yellow-100 dark:bg-yellow-900/30 dark:text-yellow-300 dark:ring-yellow-500/30 dark:hover:bg-yellow-900/50" wire:navigate>
                                    Bewerken
                                </a>
                                <form action="{{ route('admin.indicators.destroy', $indicator) }}" method="POST" onsubmit="return confirm('Weet je zeker dat je deze indicator wilt verwijderen?')">
                                    @csrf
                                    @method('DELETE')
