    ...
    <div class="space-y-6">
        ...
        @else
                ...
                @if($teams->count() <= 3)
                    <div class="flex flex-wrap gap-1 bg-slate-100 p-1 rounded-xl w-fit">
                        @foreach($teams as $team)
                            <a href="{{ route('teacher.action-points.index', ['team' => $team->id, 'filter' => $filter !== 'all' ? $filter : null]) }}"
                ...
                @else
                    <div class="flex items-center gap-3">
                        ...
                        <select id="team-select"
                                onchange="window.location='{{ route('teacher.action-points.index') }}?team='+this.value+'{{ $filter !== 'all' ? '&filter='.$filter : '' }}'"
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a href="{{ route('teacher.action-points.index', array_filter(['team' => $activeTeam?->id, 'user' => $selectedUser?->id])) }}"
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <select id="user-filter"
                        ...
                        onchange="(function(sel){
                            var params = {};
                            var team = sel.getAttribute('data-team');
                            var filter = sel.getAttribute('data-filter');
                            if (team) params.team = team;
                            if (filter) params.filter = filter;
                            if (sel.value) params.user = sel.value;
            ...
            @forelse($actionPoints as $ap)
                @php
                    ...
                    $badgeMap = [
                        ...
                        'Op schema'     => 'bg-blue-100 text-blue-700 border border-blue-300',
                        ...
                        'Afgerond'      => 'bg-emerald-100 text-emerald-700 border border-emerald-300',
                    ];
            ...
            @empty
                <div class="bg-white border-2 border-slate-200 rounded-xl p-12 text-center">
                    <p class="text-slate-400 text-sm">
                        Geen actiepunten gevonden{{ $filter !== 'all' ? ' voor dit filter' : '' }}{{ $selectedUser ? ' voor '.$selectedUser->name : '' }}.
                    </p>
                </div>
            @endforelse
