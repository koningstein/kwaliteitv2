<x-teacher-layout>
    ...
    <div class="space-y-6">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Overzicht van teamleden en hun actiepunten</p>
        </div>

        @if($users->isEmpty())
            <div class="bg-white border-2 border-slate-200 rounded-xl p-12 text-center">
                <p class="text-slate-400 text-sm">Geen gebruikers gevonden.</p>
            </div>
        @else
            ...
            @php
                ...
                foreach ($users as $user) {
                    foreach ($user->actionPoints as $ap) {
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                @foreach($users as $user)
                    ...
                    <div class="bg-white border-2 rounded-xl p-5 hover:shadow-md transition-all {{ $hasOverdue ? 'border-red-200' : 'border-slate-200 hover:border-slate-300' }}">
                        ...
                        {{-- Link naar gefilterde actiepunten --}}
